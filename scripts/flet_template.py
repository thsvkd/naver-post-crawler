...
1) 설치기가 "설치가 부분적으로 성공했습니다" 경고를 띄운다.
   ...
   자동 업데이트는 앱 안의 ``UpdateManager``가 따로 하므로 대부분의 훅에서는 할 일이 없다.
   **예외는 제거 훅이다** — ``--veloapp-uninstall``에서 자격 증명 관리자에 저장된 세션
   쿠키를 지운다. 파이썬이 뜨지 않으므로 이 삭제는 여기서만 할 수 있고, 따라서 이 파일이
   그 로직의 유일한 소재지다(:data:`_CRED_DELETE_LINES`).
...
def cache_dir_name(
    flet_version: str, *, width: int, height: int, targets: tuple[str, ...] = WINDOWS_TARGETS
) -> str:
    """패치된 템플릿을 캐시할 디렉터리 이름.

    flet 버전뿐 아니라 **패치 리비전·창 크기·자격 증명 항목 이름까지** 이름에 넣는다. flet은
    템플릿의 내용이 아니라 경로/버전만 해시해 Flutter 프로젝트 재생성 여부를 정하므로, 경로가
    그대로면 패치를 고쳐도 옛 결과물로 조용히 빌드된다(:data:`_PATCH_REVISION` 주석 참고).

    항목 이름까지 넣는 이유: ``credentials.py``의 SERVICE/ACCOUNT를 바꾸면 main.cpp에 박히는
    값이 달라지는데, 리비전을 올리는 걸 잊으면 **옛 이름을 지우는 제거 훅이 그대로 배포된다**.
    그건 아무것도 지우지 않고 조용히 성공하므로 실기로 제거해 보기 전엔 드러나지 않는다.
    """
    digest = hashlib.sha256("|".join(targets).encode("utf-8")).hexdigest()[:8]
    return f"{flet_version}-r{_PATCH_REVISION}-{width}x{height}-{digest}"


def cache_stamp_key(*, width: int, height: int, targets: tuple[str, ...] = WINDOWS_TARGETS) -> str:
    """캐시 디렉터리 안에 남기는 스탬프 값.

    :func:`cache_dir_name`과 **같은 입력에 함께 반응해야 한다**. 한쪽만 반응하면 캐시가
    "맞다"고 판정해 패치가 빠진 템플릿을 그대로 재사용한다.
    """
    return f"{_PATCH_REVISION}|{width}x{height}|{'|'.join(targets)}"
