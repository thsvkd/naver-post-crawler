...
def app_data_dir() -> Path:
    ...
    ``FLET_APP_STORAGE_DATA``가 있으면 그것을 쓰고, 없으면 플랫폼별 사용자 데이터 경로를 쓴다.

    .. note::
        **이 변수는 개발 중에만 선다.** ``flet run``(``flet_cli/commands/run.py``)이 자식
        프로세스 환경에 넣어 주는 값이고, 가리키는 곳은 ``<프로젝트>/storage/data``다.
        빌드된 배포본에는 이 변수를 세우는 코드가 없다(실측: 만들어진 ``.app`` 전체를
        뒤져도 문자열 자체가 없다). 즉 **사용자 머신에서는 항상 아래 플랫폼별 경로로
        떨어진다.**

        그래서 이 분기는 "프로덕션 경로 선택"이 아니라 **개발 편의**다 — ``flet run``으로
        돌릴 때 설정·로그가 실제 사용자 데이터 폴더를 어지럽히지 않는다. 대신 개발 중에는
        아래 경로 로직과 이관 코드가 실행되지 않으므로, 그쪽을 고칠 때는 이 변수를 비우고
        확인해야 한다.
